...
void main() {
  test('nearestMunicipalities builds the expected query', () async {
    final client = MockClient((request) async {
      ...
      return http.Response(jsonEncode({'data': [{'slug': 'tripoli'}]}), 200);
    });
    final api = LibyaDevApi(baseUrl: 'https://api.example.ly/api/v1/', client: client);
  ...
  test('convertCurrency sends JSON and raises API exceptions', () async {
    final client = MockClient((request) async {
      ...
      return http.Response(jsonEncode({'message': 'unsupported_currency'}), 422);
    });
    final api = LibyaDevApi(baseUrl: 'https://api.example.ly/api/v1', client: client);
