...
class LibyaDevApi {
  ...
  Future<Map<String, dynamic>> municipalities([String? q]) =>
  ...
  Future<Map<String, dynamic>> municipalityPoints({bool mappedOnly = false}) =>
      _request(
        '/geo/municipality-points',
        query: {'mapped_only': mappedOnly ? 1 : null},
      );

  Future<Map<String, dynamic>> nearestMunicipalities(
    double lat,
    double lng, {
    int limit = 5,
  }) => _request(
  ...
  Future<Map<String, dynamic>> validatePhone(String phone) =>
      _request('/phone/validate', method: 'POST', body: {'phone': phone});

  Future<Map<String, dynamic>> telecomOperators() =>
      _request('/telecom/operators');

  Future<Map<String, dynamic>> banks({String? q, String? city}) =>
      _request('/banks', query: {'q': q, 'city': city});

  Future<Map<String, dynamic>> bank(String slug) =>
      _request('/banks/${Uri.encodeComponent(slug)}');

  Future<Map<String, dynamic>> holidays([int? year]) =>
      _request(year == null ? '/holidays' : '/holidays/$year');
  ...
  Future<Map<String, dynamic>> nextBusinessDay(String date) =>
